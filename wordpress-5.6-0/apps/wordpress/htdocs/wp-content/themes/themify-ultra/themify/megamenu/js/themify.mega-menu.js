    ...
    const cacheMenu = {},
		 ev = Themify.isTouch?'click':'mouseover',
            init = function (e) {
				...
				let el=e.type==='mouseenter'?this.getElementsByClassName('mega-link')[0]:e.target;
				...
				let megaMenuPosts = wrapper[0].getElementsByClassName('mega-menu-posts')[0];
				if(!megaMenuPosts){
					megaMenuPosts = document.createElement('div');
					megaMenuPosts.className = 'mega-menu-posts tf_left tf_box';
					wrapper[0].appendChild(megaMenuPosts);
				}
