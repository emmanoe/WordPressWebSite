...
;
	...
	public static function sticky_element_props($props,$fields_args){
		if(isset( $fields_args['stick_at_check'] ) && ''!==$fields_args['stick_at_check']){
			...
			if($is_sticky!==false){
				$settings=array('stick'=>array());
				if(isset( $fields_args['stick_at_position'] ) && $fields_args['stick_at_position']==='bottom'){
					$settings['stick']['position']=$fields_args['stick_at_position'];
				}
				if(!empty( $fields_args['stick_at_pos_val'] )){
					$settings['stick']['value']=$fields_args['stick_at_pos_val'];
				}

				if(isset( $fields_args['stick_at_pos_val_unit'] ) && $fields_args['stick_at_pos_val_unit']!=='px'){
					$settings['stick']['val_unit']=$fields_args['stick_at_pos_val_unit'];
				}

				if(isset( $fields_args['unstick_when_check'] ) && ''!==$fields_args['unstick_when_check'] && false!==$fields_args['unstick_when_check']){
					$unstick=array();
					if(isset( $fields_args['unstick_when_condition'] ) && $fields_args['unstick_when_condition']!=='hits'){
						$unstick['rule']=$fields_args['unstick_when_condition'];
					}
					if(isset( $fields_args['unstick_when_pos'] ) && $fields_args['unstick_when_pos']!=='this'){
						$unstick['current']=$fields_args['unstick_when_pos'];
					}
					if(isset( $fields_args['unstick_when_element'] ) && $fields_args['unstick_when_element']!=='builder_end'){
						$unstick['el_type']=$fields_args['unstick_when_element'];
						if($fields_args['unstick_when_element']==='row' && $fields_args['unstick_when_el_row_id']!=='row'){
							$unstick['el_row_target']=$fields_args['unstick_when_el_row_id'];
						}
					}else{
						$unstick['el_type']='builder_end';
                    }
					if(isset( $fields_args['unstick_when_pos_val_unit'] ) && $fields_args['unstick_when_pos_val_unit']!=='px'){
						$unstick['val_unit']=$fields_args['unstick_when_pos_val_unit'];
					}
					if(!empty( $fields_args['unstick_when_pos_val'] )){
						$unstick['value']=$fields_args['unstick_when_pos_val'];
					}
					if(!empty( $fields_args['unstick_when_el_mod_id'] ) && ( !isset( $unstick['el_type'] ) || $unstick['el_type']!=='row' )){
						$unstick['el_mod_target']=$fields_args['unstick_when_el_mod_id'];
					}
					if(!empty( $unstick )){
						$settings['unstick']=$unstick;
						unset( $unstick );
					}
				}
